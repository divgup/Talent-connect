...
class Venue(db.Model):
    ...
    #venues = db.Column(db.PickleType())
...
def venues():
  ...
  today = datetime.now()
  for area in areas:
    result = Venue.query.filter(Venue.city==area.city).filter(Venue.state==area.state).all()
    Venues_list = []
    today = datetime.now()
    for venue in result:
      Venues_list.append({'id': venue.id,
      'name' : venue.name,
      'num_upcoming_shows': len(Show.query.filter_by(venue_id=venue.id).filter(Show.start_time > today).all())
      })
      
    data.append({'venues' : Venues_list,
    'city' : area.city,
    'state' : area.state})   
  #data = db.session.query(Venue).group_by(Venue.name).all()
  # data=[{
  #   "city": "San Francisco",
  #   "state": "CA",
  #   "venues": [{
...
def search_venues():
  ...
  sub_string = request.form.get('search_term', '')
  #result = Venue.query.filter(Venue.name.contains(sub_string))
  result = db.session.query(Venue).filter(Venue.name.ilike(f'%{sub_string}%')).all()  
  #cout << result.
  count = len(result)
...
def show_venue(venue_id):
  ...
  response =  {
    ...
    "upcoming_shows_count":len(up_show)
...
def search_artists():
  ...
  sub_string = request.form.get('search_term', '')
  result = db.session.query(Artist).filter(Artist.name.ilike(f'%{sub_string}%')).all()
  count = len(result)
...
def show_artist(artist_id):
  ...
  upcoming = db.session.query(Show).filter(Show.artist_id==artist_id).filter(Show.start_time > datetime.now()).join(Venue,Show.venue_id==Venue.id).add_columns(Venue.id,Venue.name,Venue.image_link,
  Show.start_time).all()
  past = db.session.query(Show).filter(Show.artist_id==artist_id).filter(Show.start_time < datetime.now()).join(Venue,Show.venue_id==Venue.id).add_columns(Venue.id,Venue.name,Venue.image_link,
  Show.start_time).all()
  up_shows  = []
  for i in upcoming:
    up_shows.append({
